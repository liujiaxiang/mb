...
import com.daymooc.fcms.common.model.Session;
...
public class LoginService
{
	...
	public Ret login(String userName, String password, boolean keepLogin, String loginIp)
	{
		...
		//先判断用户使用的是邮箱还是用户名
		//如果用户名包含@,则是邮箱登录，在注册的时候对用户名进行过校验，用户名不能包含@
		if (userName.contains("@"))
		{
			loginAccount = userDao.findFirst("select * from user where email = ? limit 1", userName);
		}
		else 
		{
			userName = userName.toLowerCase().trim();//对于用户名，统一进行小写处理
			loginAccount = userDao.findFirst("select * from user where lower(userName) = ? limit 1", userName);
		}
		...
		{
			return Ret.fail("msg","用户已经被锁定");
		}
		
		if (loginAccount.isStatusReg())
		{
			return Ret.fail("msg", "账号已经注册，但未激活，请先激活账号");
		}
	...
	public Ret sendRetrievePasswordAuthEmail(String email) {
		...
		User account = userDao.findFirst("select * from user where email=? limit 1", email);
		if (account == null) {
		...
		try {
			...
			return Ret.fail("msg", "密码找回邮件发送失败，可能是邮件服务器出现故障，请去德眼视频官方QQ群留言给群主");
		}
	}
	...
	public Ret retrievePassword(String authCodeId, String newPassword) {
		...
		if (authCode != null && authCode.isValidRetrievePasswordAuthCode()) {
			...
			int result = Db.update("update user set password=?, salt=? where id=? limit 1", newPassword, salt, accountId);
			if (result > 0) {
	...
	public void reloadLoginAccount(User loginAccountOld) {
		...
		User loginAccount = userDao.findFirst("select * from user where id=? limit 1", loginAccountOld.getId());
		loginAccount.removeSensitiveInfo();               // 移除 password 与 salt 属性值
		loginAccount.put("sessionId", sessionId);        // 保存一份 sessionId 到 loginAccount 备用
